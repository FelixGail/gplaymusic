...
public class SearchResponse implements Serializable {
  ...
  public List<Track> getTracks() {
    return entries.stream().filter(Track.class::isInstance).map(Track.class::cast)
        .collect(Collectors.toList());
  }

  public List<Artist> getArtists() {
    return entries.stream().filter(Artist.class::isInstance).map(Artist.class::cast)
        .collect(Collectors.toList());
  }

  public List<Album> getAlbums() {
    return entries.stream().filter(Album.class::isInstance).map(Album.class::cast)
        .collect(Collectors.toList());
  }

  public List<Playlist> getPlaylists() {
    return entries.stream().filter(Playlist.class::isInstance).map(Playlist.class::cast)
        .collect(Collectors.toList());
  }

  public List<Station> getStations() {
    return entries.stream().filter(Station.class::isInstance).map(Station.class::cast)
        .collect(Collectors.toList());
  }

  public List<Situation> getSituations() {
    return entries.stream().filter(Situation.class::isInstance).map(Situation.class::cast)
        .collect(Collectors.toList());
  }

  public List<Video> getVideos() {
    return entries.stream().filter(Video.class::isInstance).map(Video.class::cast)
        .collect(Collectors.toList());
  }

  public List<PodcastSeries> getPodcastSeries() {
    return entries.stream().filter(PodcastSeries.class::isInstance).map(PodcastSeries.class::cast)
        .collect(Collectors.toList());
  }
