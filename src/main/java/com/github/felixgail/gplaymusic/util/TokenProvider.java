...
public class TokenProvider {
   ...
   * @param password Password of the account. If you are using two-factor authentication, you will
   * need to create an app password <a href="https://support.google.com/accounts/answer/185833">here</a>.
   * @param androidID An android ID connected with your google account.
   * @return an {@link svarzee.gps.gpsoauth.AuthToken}. This token does not expire, therefore
   * changing the String inside {@link svarzee.gps.gpsoauth.AuthToken#getToken()} could fasten your
   * next startup.
  ...
  /**
   * Generate a token from a saved one. Use this function with care, as PlayMusic tokens have no
   * expiry date - but can be invalidated if (for example) the same android id was used in the
   * meantime to generate a new token.
   *
  ...
  /**
   * Returns the time of the last {@link #provideToken(String, String, String)} call in
   * Milliseconds.
   */
