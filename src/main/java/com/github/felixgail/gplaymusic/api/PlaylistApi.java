...
public class PlaylistApi implements SubApi{
    ...
    private List<PlaylistEntry> getContentsForSharedPlaylist
        ...
        List<PlaylistEntry> entries = mainApi.getService()
                .listSharedPlaylistEntries(requestBody).execute().body().toList();
        entries.forEach(e -> e.setApi(entryApi));
        return entries;
    }

    public List<Playlist> listPlaylists()
            throws IOException {
        List<Playlist> playlists = new PagingHandler<Playlist>() {
            @Override
        ...
        playlists.forEach(p -> p.setApi(this));
        return playlists;
    }
